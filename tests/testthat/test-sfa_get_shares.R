for (sfplus in c(TRUE, FALSE)) {
  ...
  test_that("sfa_get_prices() works for type = common", {
    ...
    ref_classes <- c(
      "integer", "character", "Date", "integer64"
    )
    names(ref_classes) <- ref_names

    res_1 <- sfa_get_shares("GOOG", type = "common")
    ...
    expect_identical(unique(res_2[["ticker"]]), c("AMZN", "GOOG"))
    expect_identical(
      res_2[["date"]],
    ...
    if (isTRUE(sfplus)) {
      ...
      res_5 <- sfa_get_shares(c("GOOG", "AMZN"), type = "common", start = date)
      checkmate::expect_data_table(
        res_5,
        key = "ticker",
        types = ref_classes,
      ...
      expect_named(res_5, ref_names)
      expect_identical(unique(res_5[["ticker"]]), c("AMZN", "GOOG"))
      expect_identical(
        res_5[["date"]],
        `attr<-`(res_5[["date"]], "label", "Date")
      )
      expect_gte(min(res_5[["date"]]), date)

      res_6 <- sfa_get_shares(c("GOOG", "AMZN"), type = "common", end = date)
      checkmate::expect_data_table(
        res_6,
        key = "ticker",
        types = ref_classes,
      ...
      expect_named(res_6, ref_names)
      expect_identical(unique(res_6[["ticker"]]), c("AMZN", "GOOG"))
      expect_identical(
        res_6[["date"]],
        `attr<-`(res_6[["date"]], "label", "Date")
      )
