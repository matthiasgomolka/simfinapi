...
test_that("sfa_load_companies works with details = FALSE/TRUE", {
    col_order <- c(
        "id", "name", "ticker", "isin", "sectorCode", "sectorName", "industryName", "market",
        "endFy", "numEmployees", "companyDescription"
    )
    for (details in c(FALSE, TRUE)) {
        entities <- sfa_load_companies(details = details)
        expect_data_table(entities,
            key = "id", types = c("integer", "character"), min.rows = 4000L, min.cols = 7L,
            max.cols = 11L, col.names = "strict"
        )
